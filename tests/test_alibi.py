...
def attention_pytorch_alibi(
    ...
    attn_weight = attn_weight + score_mod(attn_weight.size(-2), attn_weight.size(-1)).to(attn_weight.dtype)
...
def main():
    ...
    assert_close(o0.to(torch.float32), o1.to(torch.float32), atol=1e-2, rtol=1e-2)
    assert_close(o0, o1)
