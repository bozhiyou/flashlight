...
# kernel path: /tmp/torchinductor_byou/oo/coovmzrzl7clhzhpvq5q6ipefgulum74fqv7jeyasx6wulux3czd.py
...
def triton_(in_out_ptr0, in_ptr0, in_ptr1, in_ptr2, xnumel, XBLOCK0: tl.constexpr, xnumbl0: tl.constexpr, XBLOCK: tl.constexpr, RBLOCK: tl.constexpr):
    ...
    # x4 = (xindex // 4096) % 32
    # x5 = (xindex // 131072)
    tmp19 = tl.full([XBLOCK0, 1], float("-inf"), tl.float32)
    # x6 = x6
    tmp27 = tl.full([XBLOCK0, 64], 0, tl.float32)
    tmp32 = tl.full([XBLOCK0, 1], 0, tl.float32)
    ...
    for roffset in range(0, 4096, RBLOCK):
        ...
        tmp25 = tl.load(in_ptr2 + ((x6)[None, :] + (64*r7)[:, None] + 262144*x4 + 8388608*x5), (x6 < 64)[None, :] & (r7 < 4096)[:, None] & (x4 < 32) & (x5 < 4), eviction_policy='evict_last', other=0.0)
        ...
        tmp15 = (r7)[None, :] + (-x0)[:, None]
        tmp16 = tmp15.to(tl.float32)
        tmp17 = tmp14 * tmp16
        tmp18 = tmp6 + tmp17
        tmp20 = tl.max(tmp18, 1)
        tmp21 = tl.maximum(tmp19, tmp20[:, None])
        tmp22 = tmp18 - tmp21
        tmp23 = tl_math.exp2((tmp22) * 1.44269504)
        tmp24 = tmp23.to(tl.bfloat16)
        tmp26 = tl.dot(tmp24, tmp25, input_precision='ieee')
        tmp28 = tmp19 - tmp21
        tmp29 = tl_math.exp2((tmp28) * 1.44269504)
        tmp30 = tmp27 * tmp29
        tmp27 = tmp30
        tmp31 = tmp27 + tmp26
        tmp27 = tmp31
        tmp33 = tmp32 * tmp29
        tmp32 = tmp33
        tmp34 = tmp32 + tl.sum(tmp23, 1)[:, None]
        tmp32 = tmp34
        tmp19 = tmp21
    tmp35 = tmp27 / tmp32
    tl.debug_barrier()
    tl.store(in_out_ptr0 + ((x6)[None, :] + (64*x0)[:, None] + 262144*x4 + 8388608*x5), tmp35, (x6 < 64)[None, :] & (x0 < 4096)[:, None] & (x4 < 32) & (x5 < 4))
''', device_str='cuda')
