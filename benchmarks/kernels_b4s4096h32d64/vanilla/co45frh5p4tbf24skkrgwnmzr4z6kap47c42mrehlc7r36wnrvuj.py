...
# kernel path: /tmp/torchinductor_byou/j2/cj2qdsji6mo3ersqv5qrdiloweemsaytab6dl7hqrzvhv3m3x5x5.py
# Topologically Sorted Source Nodes: [matmul, attn_weight, attn_weight_1], Original ATen: [aten.bmm, aten.mul, aten._softmax]
# Source node to ATen node mapping:
#   attn_weight => mul
#   attn_weight_1 => amax, exp, sub, sum_1
#   matmul => bmm
# Graph fragment:
...
triton_blo_fused__softmax_bmm_mul_0 = async_compile.triton('triton_', '''
import triton
import triton.language as tl
...
@triton_heuristics.blockreduction(
    ...
    inductor_meta={'autotune_hints': set(), 'kernel_name': 'triton_blo_fused__softmax_bmm_mul_0', 'mutated_arg_names': ['in_out_ptr0'], 'no_x_dim': False, 'num_load': 3, 'num_reduction': 4, 'backend_hash': 'A9C866B4A14FD3277824029365D703C2427B2E685E54EC9B3EF4ADC8D1EEAC1D', 'are_deterministic_algorithms_enabled': False, 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': True, 'autotune_remote_cache': None, 'force_disable_caches': False, 'dynamic_scale_rblock': True, 'max_autotune': False, 'max_autotune_pointwise': False, 'min_split_scan_rblock': 256, 'spill_threshold': 16, 'store_cubin': False, 'block_args': {'xnumel': 4096, 'xnumbl0': 4096, 'RBLOCK': 4096}}
)
@triton.jit
def triton_(in_out_ptr0, in_ptr0, in_ptr1, in_ptr2, xnumel, XBLOCK0: tl.constexpr, xnumbl0: tl.constexpr, XBLOCK: tl.constexpr, RBLOCK: tl.constexpr):
    ...
    # x6 = x6
    tmp15 = tl.full([XBLOCK0, 64], 0, tl.float32)
    ...
    for roffset in range(0, 4096, RBLOCK):
        ...
        tmp13 = tl.load(in_ptr2 + ((x6)[None, :] + (64*r7)[:, None] + 262144*x4 + 8388608*x5), (x6 < 64)[None, :] & (r7 < 4096)[:, None] & (x4 < 32) & (x5 < 4), eviction_policy='evict_last', other=0.0)
    ...
    tmp23 = tmp15 / tmp20
    tl.debug_barrier()
    tl.store(in_out_ptr0 + ((x6)[None, :] + (64*x0)[:, None] + 262144*x4 + 8388608*x5), tmp23, (x6 < 64)[None, :] & (x0 < 4096)[:, None] & (x4 < 32) & (x5 < 4))
''', device_str='cuda')
...
def call(args):
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        buf4 = empty_strided_cuda((128, 4096, 64), (262144, 64, 1), torch.bfloat16)
        # Topologically Sorted Source Nodes: [matmul, attn_weight, attn_weight_1], Original ATen: [aten.bmm, aten.mul, aten._softmax]
        stream0 = get_raw_stream(0)
        triton_blo_fused__softmax_bmm_mul_0.run(buf4, arg0_1, arg1_1, arg2_1, 524288, grid=grid(524288), stream=stream0)
        del arg0_1
        del arg1_1
        del arg2_1
        del buf2
