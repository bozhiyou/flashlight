...
if __name__ == "__main__":
    ...
    # dist.init_process_group(backend="nccl") # torchrun
    ...
    embed_dim = 2048
    depth = 12
    # baseline Transformer's num_heads set to twice
    # baseline_num_heads = num_heads * 2
    # head_dim = embed_dim // baseline_num_heads

    print("creating test data")
    # rel_pos = init_rope(seq_len, head_dim, device)
    hidden_states = torch.randn((batch_size, seq_len, embed_dim), device=device, dtype=dtype)
    # q = torch.randn((batch_size, baseline_num_heads, seq_len, embed_dim // num_heads // 2), device=device, dtype=dtype)
    # k = torch.randn((batch_size, baseline_num_heads, seq_len, embed_dim // num_heads // 2), device=device, dtype=dtype)
    # v = torch.randn((batch_size, baseline_num_heads, seq_len, embed_dim // num_heads), device=device, dtype=dtype)
    # torch.Size([2, 32, 512, 64])  (1048576, 32768, 64, 1)
    # torch.Size([2, 32, 512, 64])  (1048576, 32768, 64, 1)
    # torch.Size([2, 16, 512, 128]) (1048576, 65536, 128, 1)
