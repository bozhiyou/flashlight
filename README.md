...
This repository contains the source code, benchmarks, and artifact evaluation (AE) scripts for the MLSys 2026 paper: *Flashlight: PyTorch Compiler Extensions to Accelerate Attention Variants*.
...
## Artifact Evaluation (MLSys'26 AE)

We provide two automated, seamless workflows to evaluate the artifact and reproduce the paper's results on a single **NVIDIA A100 or H100 GPU**. Both workflows ultimately run the full benchmark suite and generate the paper's figures (Figures 2, 3, and 4) in `benchmarks/results/`.

### Option A: Local run (via `uv`, Recommended for isolated machines)
...
```bash
./scripts/run_mlsys26_ae_local.sh
```
...
TORCH_WHL_INDEX_URL=https://download.pytorch.org/whl/cu121 ./scripts/run_mlsys26_ae_local.sh

# Simulated frequency capping target (defaults to 1290 MHz)
FL_GPU_CLOCK_FREQ_MHZ=1290 ./scripts/run_mlsys26_ae_local.sh
```

### Option B: Apptainer (Recommended for shared clusters like TACC)

If you are evaluating on a cluster where Apptainer is preferred, you can build and run the provided container. For example, on TACC Lonestar6:
...
See [apptainer/README.md](apptainer/README.md) for more details on the containerized workflow.
...
- **Frequency Capping:** To accurately reproduce the reported speedups, locking the GPU SM clock frequency is strongly recommended. The automated scripts use a fallback heuristic (`FL_GPU_CLOCK_FREQ_MHZ=1290`) to "warm up" the GPU to the target frequency when `sudo` is unavailable (e.g., on TACC). While this improves consistency, it is best-effort and may still introduce minor discrepancies.
...
If you wish to explore the code outside the automated AE workflows, you can install the package and its dependencies manually.
